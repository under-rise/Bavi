...
{
	return preg_replace('/<a(.*)>(.*)<\/a>/iU', sprintf('<span class="more">$2</span>', get_the_title(get_the_ID())), $html);
	//	return preg_replace( '/<a(.*)>(.*)<\/a>/iU', sprintf( '<span class="more"><a$1>$2</a></span>', get_the_title( get_the_ID() ) ), $html );
}
...
function img_figure($html, $id, $alt, $title, $align, $size)
{
	$html = '<figure>' . "\n" . $html . "\n" . '</figure>' . "\n";
	return $html;
}
add_filter('get_image_tag', 'img_figure', 10, 8);
